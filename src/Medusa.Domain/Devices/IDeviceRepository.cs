...
public interface IDeviceRepository
{
    ...
    Task<DeviceActivity?> GetDeviceActivityAsync(Guid activityId, CancellationToken cancellationToken = default);

    /// <summary>
    ///     Adds a new device activity.
    /// </summary>
    /// <param name="activity">The <see cref="DeviceActivity" /> object representing the activity to add.</param>
    /// <param name="cancellationToken">The cancellation token to observe while waiting for the task to complete.</param>
    /// <returns>The added <see cref="DeviceActivity" /> object.</returns>
    Task<DeviceActivity> AddDeviceActivityAsync(DeviceActivity activity, CancellationToken cancellationToken = default);

    /// <summary>
    ///     Updates an existing device activity.
    /// </summary>
    /// <param name="activity">The <see cref="DeviceActivity" /> object representing the device activity to update.</param>
    /// <param name="cancellationToken">The cancellation token to observe while waiting for the task to complete.</param>
    Task UpdateDeviceActivityAsync(DeviceActivity activity, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">The cancellation token to observe while waiting for the task to complete.</param>
    /// <returns>A boolean indicating whether the deletion was successful.</returns>
    Task<bool> DeleteDeviceActivityAsync(Guid activityId, CancellationToken cancellationToken = default);
}
